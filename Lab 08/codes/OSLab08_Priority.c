...
// sort processes according to their priority
void sort(struct process* processes) {
    for (int i=0; i<n-1; i++)
        for (int j=0; j<n-i-1; j++)
            if (processes[j].pri < processes[j+1].pri)
...
int main() {
    ...
    for (int i=0 ; i<n; i++) {
		...
		printf("\nProcess %d:\n", processes[i].pid);
		printf("  Burst Time = %d - Priority = %d - Waiting Time = %d - Turnaround Time = %d\n", 
			processes[i].bt, processes[i].pri, processes[i].wt,  processes[i].tt);
    }
